...
def test_parse_alert_no_classtype():
    count = 0
    for x in suricata.parse_alert("01/28/2014-22:26:04.885446 [**] [1:1917:11] INDICATOR-SCAN UPnP service discover attempt [**] [Priority: 3] {UDP} 10.1.1.132:58650 -> 239.255.255.250:1900\n"):
        assert x['message'] == "INDICATOR-SCAN UPnP service discover attempt"
        assert x['timestamp'] == datetime(2014, 1, 28, 22, 26, 4, 885446)
        assert x['sid'] == 1917
        assert x['revision'] == 11
